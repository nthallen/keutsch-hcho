...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-CH2O-install.ps1
#
# ---------
# Uninstall:
# ---------
...
# > PowerShell -ExecutionPolicy Bypass ./cygwin-monarch-CH2O-install.ps1 -uninstall
...
'@
$setup_script | Out-File -FilePath "./monarch-CH2O-install.sh" -NoNewLine -Encoding ASCII

Write-Output "`nStarting standard install script: /usr/local/src/monarch-CH2O-install.sh`n"

# -Wait won't work here, because mintty.exe doesn't really exit until ssh-agent does
start-process C:\cygwin64\bin\mintty.exe -argumentlist "-h always /bin/bash --login /usr/local/src/monarch-CH2O-install.sh -S $exp_option"
